...
- (void)determineType {
    ...
    NSRegularExpression *regexpDocomo = [NSRegularExpression
                                         regularExpressionWithPattern:patternDocomo
                                         options:0 error:nil];
    
    NSTextCheckingResult *matchDocomo = [regexpDocomo
                                         firstMatchInString:barcodeString options:0
                                         range:NSMakeRange(0, barcodeString.length)];
    
    //AU/Softbankの場合の正規表現
    NSString *patternAUSB = @"^(MEMORY:)";
    
    NSRegularExpression *regexpAUSB = [NSRegularExpression
                                       regularExpressionWithPattern:patternAUSB
                                       options:0 error:nil];
    
    NSTextCheckingResult *matchAUSB = [regexpAUSB
                                       firstMatchInString:barcodeString options:0
                                       range:NSMakeRange(0, barcodeString.length)];
    
    //双方一致しない場合
    if (matchDocomo.numberOfRanges == 0 &&
        matchAUSB.numberOfRanges == 0) {
        barcodeResults.text =
        @"連絡先では無いバーコードが読み込まれました。";
        
        //Docomoに一致する場合
    } else if (matchDocomo.numberOfRanges > 0 &&
               matchAUSB.numberOfRanges == 0) {
        //Docomoのフォーマットに従って解析
...
- (Contact *)parseDocomo {
    ...
    //「;」で項目ごとに分割
    NSArray* codeElement = [barcodeString
    ...
    NSRegularExpression *regexpName = [NSRegularExpression
                                       regularExpressionWithPattern:patternName
                                       options:0 error:nil];
    NSRegularExpression *regexpYomi = [NSRegularExpression
                                       regularExpressionWithPattern:patternYomi
                                       options:0 error:nil];
    NSRegularExpression *regexpEmail = [NSRegularExpression
                                        regularExpressionWithPattern:patternEmail
                                        options:0 error:nil];
    NSRegularExpression *regexpPhone = [NSRegularExpression
                                        regularExpressionWithPattern:patternPhone
                                        options:0 error:nil];
    ...
    for (int i = 0; i < [codeElement count]; i++) {
        ...
        //名前を抽出
        match = [regexpName firstMatchInString:element
                                       options:0 range:NSMakeRange(0, element.length)];
        if (match.numberOfRanges > 0) {
            NSString *name =
            [element substringWithRange:[match rangeAtIndex:1]];
            NSArray *names = [name componentsSeparatedByString:@","];
            c.lastname = [names objectAtIndex:0];
            c.firstname = [names objectAtIndex:1];
        }
        //よみがなを抽出
        match = [regexpYomi firstMatchInString:element
                                       options:0 range:NSMakeRange(0, element.length)];
        if (match.numberOfRanges > 0) {
            NSString *yomi = [element substringWithRange:
        ...
        //メールアドレスを抽出
        match = [regexpEmail firstMatchInString:element
                                        options:0 range:NSMakeRange(0, element.length)];
        
        if (match.numberOfRanges > 0) {
            NSString *email = [element
                               substringWithRange:[match rangeAtIndex:1]];
            [c.email addObject:email];
        }
        
        //電話番号を抽出
        match = [regexpPhone firstMatchInString:element
                                        options:0 range:NSMakeRange(0, element.length)];
        
        if (match.numberOfRanges > 0) {
            NSString *phone = [element 
                               substringWithRange:[match rangeAtIndex:1]];
